...
// ---------------------------------------------------------------------------
// Page component — PDF render target only, no ProposalFAB, no scroll-snap
// ---------------------------------------------------------------------------
...
export default async function ProposalPdfPage({
  ...
  // Return simple error instead of redirect — Playwright hitting a redirect would fail silently
  if (!params.d) {
    return <div>No proposal data</div>;
  }

  // Decode form values — return error on invalid base64
  let formValues: ProposalFormValues;
  try {
    formValues = decodeProposalParams(params.d);
  } catch {
    return <div>Invalid proposal data</div>;
  }
